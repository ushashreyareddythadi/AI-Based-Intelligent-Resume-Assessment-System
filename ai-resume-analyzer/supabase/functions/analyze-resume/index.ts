...
serve(async (req) => {
  ...
  try {
    ...
    const { resumeText: initialText, resumeBase64, jobRole, jobDescription } = body;
    ...
    const prompt = `Analyze this resume and provide detailed feedback:\n\n${resumeText}\n\n${context}\n\nRespond with a JSON object containing: atsScore (0-100), skillMatchPercentage (0-100), matchedSkills (array), missingSkills (array), grammarIssues (count), toneAnalysis (string), suggestions (array), marketDemand (string).`;
